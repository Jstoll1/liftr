...
async function kickoffsFor(env, week) {
  ...
  if (wk && Array.isArray(wk.games) && wk.games.length) {
    const times = wk.games.map((g) => new Date(g.kickoff).getTime());
    const first = Math.min(...times);
    return Object.fromEntries(wk.games.map((g, i) => [String(g.id), week >= LOCK_ALL_FROM_WEEK ? first : times[i]]));
  }
  return week === 1 ? PICKS_KICKOFF : {};
}

async function tiebreakerLocked(env, week, kickoffs, now) {
  const wk = await env.LIFTR_KV.get(gamesKey(week), "json");
  const tbGame = (wk?.games || []).find((g) => g.tiebreakerGame);
  const at = tbGame ? kickoffs[String(tbGame.id)] : null;
  return !!at && now >= at;
}
...
async function handlePicks(request, env, corsHeaders, url) {
  ...
  if (request.method === "POST" || request.method === "PUT") {
    ...
    try {
      ...
      // The guess locks with its game. Under the amendment that is the
      // week's first kickoff, the same as every pick.
      const tbLocked = await tiebreakerLocked(env, week, kickoffs, now);
...
function buildRecap(games, rows, actualTotal, tbGame, movement) {
  ...
  // Pick of the week is about the team, not the line. Three people on
  // Georgia State, two straight up and one at +8.5, is one bold call by
  // three people, and the straight-up pair earned the most for it. So:
  // the winning team fewest people were on, then the best-paid way to
  // have been on it, then the bigger spread.
  const teams = new Map();
  for (const l of all) {
    const k = `${l.g}|${l.team}`;
    if (!teams.has(k)) teams.set(k, { takers: 0, hits: [] });
    const t = teams.get(k);
    t.takers += l.who.length;
