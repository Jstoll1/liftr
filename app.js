...
function openAdmin() {
  ...
  tag.src = "admin.js?v=202609172030";
...
function openAppConsole() {
  ...
  const tag = document.createElement("script");
  tag.src = "console.js?v=202609172030";
...
function playerBreakdownHtml(name, state, results, live) {
  ...
  const rows = ordered.map((game) => {
    ...
    if (!locked) {
      ...
      pickHtml = `<span class="rd-pickcard"><span class="rd-pickteam"><img class="rd-logo" src="${logoUrl(pickId)}" alt="" loading="lazy" onerror="this.style.visibility='hidden'" /><span class="rd-team">${short}</span></span><span class="rd-pickmeta">${mode}${pickTimeLabel(pick) ? `<span class="rd-when"><i>PICKED</i> ${pickTimeLabel(pick)}</span>` : ""}</span></span>`;
      const pts = scorePick(game, pick, results[game.id]);
      if (pts !== null) {
        banked += pts;
